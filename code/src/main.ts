...
async function makeFile(f: string) {
	...
	if (isFolderMod) {
		for (const f1 of await readdir(join("dist", "content", f))) {
			return await makeFile(join(f, f1))
		}
	}
	...
	if (f.endsWith(".entity.patch.js")) {
		for (const _option of mod.OPTIONS ?? [null]) {
			...
			await patch.save(join(chunkFolder, `${joinOr(mod.NAME_PREFIX, mod.NAME, "patch")}.entity.patch.json`))
		}
	}
	if (f.endsWith(".entity.js")) {
		for (const _option of mod.OPTIONS ?? [null]) {
			const entity = tryFn(
				() => createEntity(mod.PATH, mod.SUBTYPE ?? "template"),
				() => {
			...
			const chunkFolder = join(CONTENT_FOLDER, joinOr(_option?.prefix, option, "content"), `chunk${mod.CHUNK ?? 0}`)
			await mkdir(chunkFolder, { recursive: true })
			if ((await mod.create(entity, option)) ?? true)
				await entity.save(join(chunkFolder, `${joinOr(mod.NAME_PREFIX, mod.NAME, "entity")}.entity.json`))
		}
	}
	...
	for (const resource of mod.RESOURCES ?? []) {
		await copyResource(resource, join(CONTENT_FOLDER, "content", `chunk${mod.CHUNK ?? 0}`), f)
	}
}

async function copyResource(resource: TResource, chunkFolder: string, mod: string) {
	await cp(
		join(...["src", "content", join(dirname(mod), (resource as any)?.from ?? resource)]),
		join(chunkFolder, (resource as any)?.to ?? resource)
	)
}
