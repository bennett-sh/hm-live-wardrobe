  ...
  if(f.endsWith('.entity.patch.js')) {
    for(const _option of mod.OPTIONS ?? [null]) {
      ...
      await patch.save(join(
        chunkFolder,
        `${joinOr(mod.NAME_PREFIX, mod.NAME, 'patch')}-${Math.round(Math.random() * 1000).toString().padStart(4, '0')}.entity.patch.json`
      ))
    }
  }
  if(f.endsWith('.entity.js')) {
    for(const _option of mod.OPTIONS ?? [null]) {
      ...
      if(await mod.create(entity, option) ?? true) await entity.save(join(
        chunkFolder,
        `${joinOr(mod.NAME_PREFIX, mod.NAME, 'entity')}-${Math.round(Math.random() * 1000).toString().padStart(4, '0')}.entity.json`
      ))
    }
  }
  if(f.endsWith('.repository.js')) {
    for(const _option of mod.OPTIONS ?? [null]) {
      ...
      if(await mod.create(repository, option) ?? true) await repository.save(join(
        chunkFolder,
        `${joinOr(mod.NAME_PREFIX, mod.NAME, 'changes')}-${Math.round(Math.random() * 1000).toString().padStart(4, '0')}.repository.json`
      ))
    }
  }
